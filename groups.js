...
{  
    ...
    { 
        ...
        var aliean = "<div id=\"creategrp\"  style=\"display:block\">"+
          "<h3> Create Groups </h3> "+
          " <form  class='create-form'>"+
              "<label>Group Name</label> <br>"+
              "<input type =\"text\" name=\"groupName\" id=\"groupName\" /> <br>"+
              "<input type =\"button\" value=\"Select Image\" name=\"groupImage\" id=\"groupImage\" /> <br> <br>"+
          "</form>"+
        "</div> "
    ...
    var allGroups = function ()
    {
        ...
        $.ajax({
            ...
            {   
                ...
                $("#allgroups").css("width","100px")
                $("#allgroups").css("display","block")

                for(var j = 0 ; j < Object.keys(RES["groupnames"]).length ; j++)
                { 
                    var group_record = "<div class='per' style='width:300px;height:300px'>";
                    group_record +=  '<img id="imgid" class="public" value="'+ RES['groupid'][j] +'" src= "/static'+RES['groupimage'][j]+ '">';
                    group_record += '<p>' +RES['groupnames'][j]  + '</p>';
                    group_record += '<button  class="join">Join</button>';
                    group_record +='</div>'
